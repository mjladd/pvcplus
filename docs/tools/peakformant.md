...
Peak formant tracker. This produces a time-series of the loudest bin's frequency over a detection band, never audio. It ports `peakformant`'s audio path. See `rust/crates/pvc-core/src/tools/peakformant.rs` for why it reuses `pvc centroid`'s whole two-pass pipeline. The two are byte-for-byte identical apart from one real per-frame analysis difference.

## Usage

```
pvc peakformant [OPTIONS] <INPUT> <OUTPUT>
```
...
| `--attack <ATTACK>` | Envelope attack time | 0 |
| `--release <RELEASE>` | Envelope release time | 0 |
...
```
pvc peakformant --band-low 200 --band-high 4000 input.wav formant.txt
```
