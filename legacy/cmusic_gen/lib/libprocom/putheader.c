/* putheader.c	1.2	(CARL)	26 Nov 1985	23:02:11 */
...
int wprop(struct proplist *p, FILE *iop)
//	struct proplist *p;
//	FILE *iop;
{
	register char *c;
	...
	if (p->propname == NULL || p->propval == NULL)
		return(-1);
	for (c = p->propname; *c != NULL_CHAR; c++) {  // NULL_CHAR WAS NULL
		if (putfc(*c, iop) != sizeof(char))
			return(-1);
	}
	if (putfc(NULL_CHAR, iop) != sizeof(char)) // NULL_CHAR WAS NULL
		return(-1);
	for (c = p->propval; *c != NULL_CHAR; c++) { // NULL_CHAR WAS NULL
		if (putfc(*c, iop) != sizeof(char))
			return(-1);
	}
	if (putfc(NULL_CHAR, iop) != sizeof(char)) // NULL_CHAR WAS NULL
