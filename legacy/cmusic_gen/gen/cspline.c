...
void readin (int argi, int argc, char **argv) {
	...
	for (n = 0; n < NP; n++) {
		if (auta)
			...
			if (argi < argc) {
				x.val[n] = atof (argv[argi++]);
				y.val[n] = atof (argv[argi++]);
			}
			else
				break;
		}
...
int main (int argc, char *argv[]) {
	...
	y.lb = INF;
	y.ub = -INF;
	while ((ch = crack (argc, argv, "a|k|n|L|ocpl|u|h", 0)) != CRACK_DONE_FLAG) {
		switch (ch) {
			case 'a': 
				auta = 1;
				dx = atof (arg_option);
			...
			case 'p': 
				...
				break;
			case 'l': 
				x.lb = atof (arg_option);
				x.lbf = 1;
				break;
			case 'u': 
				x.ub = atof (arg_option);
				x.ubf = 1;
				break;
			default: 
				fprintf (stderr, "Bad flag argument\n");
			case 'h':
				usage(1);
		}
	}
