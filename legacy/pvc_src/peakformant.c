...
//fprintf( stderr, "\nN = %d, lowf = %f, hif = %f, fundamental = %f", N,  lowf, hif, fundamental ) ; 
      ...
      findFreqOfPeakFormant( channel, (N + 2),  &lowf, &hif, &fundamental, &temp2, &temp ) ;   


//prf( temp,  "PEAK FORMANT FREQUENCY" ); 
...
// WARP
    ...
    pri( mm, "NUMBER OF VALUES IN OUTPUT FORMANT-TRACK ENVELOPE" ) ; 

    fprintf(stderr,"\n\nPEAK FORMANT : TRACKING COMPLETED\n");
...
{
    fprintf(stderr, "%s",
	"peakformant:  peak formant tracker\n"
	"peakformant [flags] [input file] [output formant track file (optional)]\n"
	...
	"	P:	Plot with gnuplot (i.e. showme <file>) 1: yes, 0: no [0]\n"
	    );
	exit(EXIT_SUCCESS);
    }
