...
float flatnessCoef, oldFlatnessCoef, flatnessCoefInDecibels, amplitudeThresholdInDecibels=-96., 
	amplitudeThreshold ;
...
strcpy( routine, "specflattracker" ) ; 
...
if( argc < 2 )usage() ; 
	...
	"R|N|m|M|P|q|w|S|D|g|t|I|b|Q|e|H|p|G|Z|X|S|f|F|L|l|d|s|a|A|c|C|T|W|n|B|X|r|p|Q|z|h", 
		0  )) != CRACK_DONE_FLAG ) {
	switch(ch) {
	    case 'm':   methodFlag = (int) crackfloat( arg_option, ch );
			break;
	    ...
	    case 'c':   amplitudeThresholdInDecibels = crackfloat( arg_option, ch ) ;
			break;
...
prline( 69,  "/" ) ; 
prline( 69,  "-" ) ; 
prbanner( "SPECTRAL FLATNESS TRACKER", 69 ) ; 
prline( 69,  "-" ) ; 
...
*******/
    ...
    if( arg_index >= argc  ){
	bannero() ;
	sprintf( ofile, "specflattracker.out" ) ; 
...
//*********************************
 ...
 if( tpinc < frames_per_sec ){
     // CHANGE IT
     prt( ".....YOUR OUTPUT TRACKING TRAJECTORY SAMPLE RATE IS < THE FRAMES PER SECOND." ) ; 
     prf( frames_per_sec,  " WILL CHANGE TO THE FRAMES PER SECOND RATE " ) ;
     tpinc =  1.  ; 
 }else{
     prf( tpinc, "OUTPUT TRACKING TRAJECTORY SAMPLES PER SECOND" ) ; 
...
// MAKE UNIQUE NAME
    	sprintf( scratch, "/tmp/flatnesstrack.%d", (int)(random()) ) ;
...
if( !hifreqUserSet ) hifreq.A[ 0 ] = nyquist ;
    ...
    amplitudeThreshold = dB_to_amp( amplitudeThresholdInDecibels ); 
...
prline( 1,  "*" ) ; 
prp( &release,  "TRAJECTORY RELEASE TIME" ) ;  
prp( &attack,  "TRAJECTORY ATTACK TIME" ) ;  
prline( 1,  "*" ) ; 
...
if( outformat == 0 ) prt("OUTPUT WILL BE IN 0-1 RANGE" ) ;
else if( outformat == 1 ) prt("OUTPUT WILL BE IN DECIBELS" ) ;
else{
...
// MAKE THE FLATNESS TRACK

	if( frame_count == 0 ){
prt( "INITIALIZING THE PREVIOUS CHANNEL . . . " ) ; 
		for( i = 0; i < (N + 2); i++ ) previous_channel[i] = channel[i] ; 
	} ; 
	find_spectralflatness( 
		&flatnessCoef, &flatnessCoefInDecibels, channel, previous_channel, (N + 2), methodFlag, 
			lowf, hif, fundamental, 
				&oldFlatnessCoef, amplitudeThreshold ) ;  

	for( i = 0; i < (N + 2); i++ ) previous_channel[i] = channel[i] ; 



//prf( flatnessCoef,  "FLATNESS" ); 
//prf( flatnessCoefInDecibels,  "FLATNESS (indB)" ); 


		
		
		//SMOOTH IT 
		if(flatnessCoef > oldFlatnessCoef){ //ATTACK
		    flatnessCoef = (attackc * oldFlatnessCoef) +
			(minusattackc * flatnessCoef) ; 
		} else { // DECAY
		    flatnessCoef = (releasec * oldFlatnessCoef) +
		    (minusreleasec * flatnessCoef) ; 
		}
		oldFlatnessCoef = flatnessCoef ; 

// OUTPUT THE AMPLITUDE 

	    if( channow == 0 ){
    		// WRITE FIRST CHANNEL
		fwrite( &flatnessCoef, sizeof(float), 1, fscratch );
	    }else{
		// READ IN VALUE
		...
		if( !chanmethod ){ 
		    // TAKE AVERAGE
		    flatnessCoef = ((( (float) channow / (float) (channow + 1) )
			* temp2 ) + flatnessCoef ) / (float)  (channow + 1) ; 
		}else{
		    // TAKE PEAK
		    if( temp2 > flatnessCoef ) flatnessCoef = temp2 ; 
		}
...
//********************************* NOW OPEN, COMPRESS AND NORMALIZE
    ...
    while( tp < 1. ){
	if( tp == 0. ) old_temp = temp ; 
	temp4 = curve( old_temp,  temp,  tp,  0.  ) ;

	// CHANGE FORMAT
	    if( outformat == 1 ){
		// TO DECIBELS
		temp4 = amp_to_dB( temp4 ) ; 
    ...
    pri( mm, "NUMBER OF VALUES IN OUTPUT TRAJECTORY" ) ; 

    fprintf(stderr,"\nSPECTRAL FLATNESS: TRACKING COMPLETED\n");
...
{
    fprintf(stderr, "%s",
	"specflattracker:  spectral flatness tracker\n"
	"specflattracker [flags] [input file] [output centroid file (optional)]\n"
	...
	"		    0 = average,  1 = peak\n"
	"	c:	amplitude threshold for excluding low valued bins; in decibels [-200]\n"
	"	?:	method: 0 = amplitude, 1 = amplitude change, \n"
	"			2 = frequency change [0]\n" 
	    ...
	    "	l:      trajectory ascent time  (func) [0.]\n"
	    "	L:      trajectory descent time   (func) [0.]\n"
	    "	W:	flatness tracker distribution warp index  [0]\n"  
	    "		    value of 0: no warp\n"
