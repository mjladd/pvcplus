...
FILTER_warpshape.L = 1. ;  FILTER_warpshape.n = 1. ; FILTER_warpshape.A[ 0 ] = 0. ; 
...
// REVERB RANDOM FREQ DEVIATION IN HZ
feedback_dither.L = 1. ; feedback_dither.n = 1. ; feedback_dither.A[ 0 ] = 0. ; 

// REVERB RANDOM FREQ DEVIATION LOWPASS FILTER RESPONSE TIME
...
// FEEDBACK RANDOM FREQ DEVIATION LOWPASS FILTER RESPONSE TIME
...
if( argc < 2 )usage() ; 
    ...
    "|=|A|B|C|D|E|F|G|H|I|J|K|L|M|N|O|P|Q|S|T|U|V|W|X|Y|Z|_|a|b|c|d|e|f|g|h|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z|", 0  )) != CRACK_DONE_FLAG ) {
	switch(ch) {
	    case 'N':   N = (int) atoi(arg_option); // crackfloat( arg_option, ch )
			break;
	    case 'M':   Nw = (int) crackfloat( arg_option, ch );
			break;
	    case 'h':   window_type = (int) crackfloat( arg_option, ch ) ;
			break;
...
// REVERB
	    ...
	    case 'z':   strcpy(tempstring, arg_option);
			...
			break;
	    case 'g':   feedback_thresh_mode = (int) crackfloat( arg_option, ch ) ;
			break;
	    ...
	    case 'q':   strcpy(tempstring, arg_option);
			FILTER_warpshape.fp = crackstring( tempstring, 
			    &FILTER_warpshape );
			break;
	   ...
	   case 'J':	strcpy(afile, arg_option); break;
			...
			break;
            case 'v':	print_flag = (int) crackfloat( arg_option, ch ) ; break;
...
prline( 69,  "/" ) ; 
prline( 69,  "-" ) ; 
prbanner( "RINGFILTER: REVERBERATOR/RESONATOR", 69 ) ; 
prline( 69,  "-" ) ; 
...
//*************WARP
//    if(warpshape != 0) prt("..............WARPING INPUT FREQUENCY RESPONSE....." ) ; 
//    spectmagwarp( F,  (N + 2), warpshape, 1 ) ;
...
//***
		...
		// RECOMPUTE FOR INTERPOLATION
		if(feedback_alowpass.A[ 0 ] <= 0.){
		    feedalowpass = 0 ; minusfeedbackalowpass =  1.  ; 
		}else{
		    feedalowpass = 
			pow( (double) ar_dB,  
			    (double) (IR / feedback_alowpass.A[ 0 ] ) ) ; 
		    minusfeedbackalowpass =  1. - feedalowpass ; 
		}
...
//**** THRESHOLD ENVELOPE GOES HERE
		...
		sourcedB.A[ 0 ] = fval( &sourcedB, dur, t );
			...
			filtamp = 1. - sourceamp ;  
		// OK. FIND THE CURRENT FILTER warpshape AND WARP BEFORE FILTERING. 
		FILTER_warpshape.A[ 0 ] = fval( & FILTER_warpshape, dur, t );
		for(i = 0; i < (N + 2); i++ ) thisF[i] = F[i] ; 
		spectmagwarp( thisF,  (N + 2), FILTER_warpshape.A[ 0 ], 1 ) ;
	   ...
	   //*** NOW FILTER IT
		...
		// FIRST MAKE INTERPOLATED FILTER AMP
		    temp = ( thisF[ i1 ] * i1p ) + (thisF[ i2 ] * i2p ) ; 

			// MAKE NEW AMP
		    feedback_channel[i - 1] = feedback_channel[i - 1] * 
			((filtamp * temp) + sourceamp );  
		}
	      }
...
// CHANNELS LOOP END
    ...
    fprintf(stderr,"\nRINGFILTER: RESYNTHESIS COMPLETED\n");
    exit(EXIT_SUCCESS) ;
...
{
    fprintf(stderr, "%s",
	"ringfilter:  phase vocoder reverberator/resonator\n"
	"		with switchable input/feedback loop filter \n"
	"ringfilter   [flags] [input file] [output file]\n"
	...
	"	h:	window type: 0 = hamming,  1 = rectangular  \n"
	...
	"	W:      REVERB random amplitude deviation floor in dB (0 -> -96) (func) [0.]\n"
	"	a:      REVERB random amplitude deviation lowpass \n"
	"			filter response time  (func) [0.]\n"

	"	    *** x(n) **** INPUT  AND EQ FILTER **\n"
	"	z:	REVERB (input) threshold in dB (0. > v >= -96.) (func) [-96.] \n"
	"	g:	REVERB threshold pass mode [1] \n"
	"		    (1 = pass bins > threshold,  0 = bins < threshold)\n"
	...
	"	v:	FILTER FREQUENCY RESPONSE PRINTOUT: high cutoff frequency in Hz [0]\n"
	"		    (0 = off)\n"
	...
	"	J:	Optional Analysis Sound File Source (for playback comparison only) [none]\n"
