...
float peakbinamp = 0.,  avgbinamp=0.,  peakamp, peakenvamp=-999999999., minenvamp=999999999., 
	ampthresh,ampgatethresh,    limit ;
float   IR,  dur=0.;
...
//  FILTERED ENVELOPE ATTACK  TIME
struct  func  fattack ; 

//  FILTERED ENVELOPE RELEASE TIME
struct  func  frelease ; 

//  FILTERED ENVELOPE CUT PROPORTION 
struct  func  fcutprop ; 
...
//  FILTERED ENVELOPE CUT PROPORTION 
fcutprop.L = 1. ; fcutprop.n = 1. ; fcutprop.A[ 0 ] = -96. ; 
...
if( argc < 2 )usage() ; 
    ...
    while( (ch= crack( argc, argv, "R|w|m|N|M|P|q|D|g|t|j|k|I|b|Q|e|p|G|Z|X|S|f|F|L|l|d|s|a|A|C|T|W|n|B|X|r|p|Q|z|h", 0  )) != CRACK_DONE_FLAG ) {
	switch(ch) {
	    ...
	    case 'm':   strcpy(tempstring, arg_option);
			fcutprop.fp = crackstring( tempstring, 
			    &fcutprop );
			break;
...
prline( 69,  "/" ) ; 
prline( 69,  "-" ) ; 
prbanner( "AMPLITUDE ENVELOPE TRACKER", 69 ) ; 
prline( 69,  "-" ) ; 
...
prline( 1,  "*" ) ; 
prp( &fattack,  "FILTERED ENVELOPE ATTACK TIME" ) ;  
prp( &frelease,  "FILTERED ENVELOPE RELEASE TIME" ) ;  
prp( &fcutprop,  "FILTERED ENVELOPE CUT PROPORTION" ) ;  
prline( 1,  "*" ) ; 
...
//*************************
		...
		frelease.A[ 0 ] = fval( &frelease, dur, t );
		    smooth_setup( frelease.A[ 0 ], &freleasec, &minusfreleasec, IR ) ;
		fattack.A[ 0 ] = fval( &fattack, dur, t );
		    smooth_setup( fattack.A[ 0 ], &fattackc, &minusfattackc, IR ) ;

		fcutprop.A[ 0 ] = fval( &fcutprop, dur, t );
...
// TURN FREQ BOUNDS INTO INDECES
		...
		i1 = 1 +  ( 2 * (int) ( lowf / freqdiff ) ) ; 
		i2 = 1 +  ( 2 * (int) ( hif / freqdiff ) ) ; 
...
// SUM THE BIN AMPS BETWEEN THE BOUNDS
		temp = 0 ; 
		for( i = i1; i <= i2; i+= 2) temp += channel[i - 1] ; 
		
		
		// CREATE FILTERED ENVELOPE
		filtenv = 
		    smooth_one_value( temp, oldfiltenv,  
			fattackc, minusfattackc, freleasec, minusfreleasec ) ; 
		oldfiltenv = filtenv ; 
		
		// SUBTRACT FROM ENVELOPE
		temp = temp - ( fcutprop.A[ 0 ] * filtenv ) ; 
		 
		//SMOOTH IT 

		temp = smooth_one_value( temp, old_temp,  
			attackc, minusattackc, releasec, minusreleasec ) ; 

		old_temp = temp ; 
		
		if( peakenvamp < temp ) peakenvamp = temp ; 
		if( minenvamp > temp ) minenvamp = temp ; 
...
prf( peakenvamp,  "PEAK ENVELOPE AMP" ) ; 
prf( minenvamp,  "MINIMUM ENVELOPE AMP" ) ; 
...
// NORMALIZE
	    temp = (temp - minenvamp) / (peakenvamp - minenvamp) ; 
...
// WARP
    ...
    if( fcutprop.n != 1. ) fclose(fcutprop.fp ) ;
...
void usage()
{
	...
	"envelope [flags] [input file] [output envelope file (optional)]\n"
	"                    (default: envelope.out)\n"
	...
	"		FILTERED ENVELOPE:\n"
	"		  (The envelope detected by the following settings can be removed\n"
	"		   with the cut proportion. Set to 1 for full removal,  0 for none.\n)"
	"	j:	FILTERED ENVELOPE: attack response time (0 to ?) (func) [-0]\n"
	"	k:	FILTERED ENVELOPE: release response time (0 to ?) (func) [-0]\n"
	"	m:	FILTERED ENVELOPE cut proportion (0 to 1) (func) [0.]\n"
